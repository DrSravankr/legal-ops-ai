/**
 * Legal Document Extractor — Google Gemini (FREE)
...
- Be concise but complete`;
    ...
    if (['pdf'].includes(ext)) {
      if (stat.size > MAX_BYTES) {
        // Large PDF — try text extraction via pdf-parse
        try {
          ...
          if (text.length > 100) {
            ...
            return `[${fileName} — ${(stat.size/1024/1024).toFixed(1)}MB — too large for Vision; text layer empty. Manually review.]`;
          }
        } catch(pe) {
...
async function extractLegalData(combinedText, reportType, bankName, firmName, fileObjects = []) {
  ...
  // Read each document — skip if cached
  // Year-wise RTCs — skip Vision reading, just note them (same format every year)
  ...
  for (const f of fileObjects) {
    ...
    if (checkpoint[fname]) {
      process.stdout.write(`  Cached: ${fname} ✓\n`);
      continue;
    }

    // Skip year-wise RTC reading — handled as a group entry
    if (RTC_YEAR_PATTERN.test(fname)) {
      ...
      process.stdout.write(`  Listed: ${fname} ✓\n`);
      continue;
    }
    ...
    try {
      ...
      // 7 second throttle — stays under 10 req/min free limit
  ...
  console.log(`\n✅ ${Object.keys(checkpoint).length} documents extracted. Structuring report...\n`);
...
WRITING STYLE FOR EACH DOCUMENT TYPE — USE EXACTLY:
...
• Sale Deed: "Sale deed registered as Document No.XXXXX/YYYY-YY, executed by [SELLER(s) with SPA/guardian details], in respect of the property bearing Sy.No.XX of [Village], [Hobli], [Taluk], [District], measuring [AREA] in favour of [BUYER]"
• Partition Deed: "Partition Deed entered between [PARTIES], in respect of their Joint Family Properties including the property bearing Sy.No.XX..., measuring [TOTAL] (land measuring [SHARE] fallen to the share of [NAME]...)"
...
• GPA: "General Power of Attorney executed by [GRANTOR] in respect of the property bearing Sy.No.XX... in favour of [GRANTEE]"
• JDA: "Joint Development Agreement executed by [LANDOWNER(S)] (Landowners) and [DEVELOPER] (Developer) in respect of the property bearing Sy.No.XX..."
...
• NOC: "NOC dated [DATE] issued by [AUTHORITY], in respect of the proposed development at Sy.No.XX of [Village]..."
• Akarband: "Akarband (Area Statement) issued by the Jurisdictional Revenue Authority in respect of the property bearing Sy.No.XX of [Village], [Hobli], [Taluk], [District]"

STRICT OUTPUT RULES:
1. CLEAN ENGLISH ONLY — absolutely no Kannada, Telugu or any Indian language characters
2. No portal URLs, no digital signature metadata, no Bhoomi/Bhoomojini references
3. No "[SCANNED]" or technical notes — write the proper legal description
